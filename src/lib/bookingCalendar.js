...
export async function changeCalendarRoomBlockStatus({
  ...
  if (!['released', 'cancelled'].includes(normalizedStatus)) {
    throw new Error('Unsupported room-block status change.')
  }

  if (!normalizedReason) {
    const actionLabel = normalizedStatus === 'released' ? 'release' : 'cancel'
    throw new Error(`A reason is required to ${actionLabel} this room block.`)
  }
  ...
  throwIfError(error)
  throwIfRejected(data, 'Unable to change room-block status.')
  notifyCalendarInvalidated({
    reason: `room_block_${normalizedStatus}`,
  })
  return data
}
