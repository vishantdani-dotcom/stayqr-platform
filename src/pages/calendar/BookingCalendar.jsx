...
export default function BookingCalendar() {
  ...
  async function handleDrop(event, roomId, arrivalDate) {
    ...
    if (payload.type !== 'reservation') return
    ...
    try {
      ...
      showNotice('error', error.message || 'The calendar move was rejected.')
      await loadCalendar({ silent: true })
    } finally {
  ...
  async function handleBlockStatus(block, status) {
    ...
    const reason = window.prompt(
      `Enter the reason to ${status === 'released' ? 'release' : 'cancel'} this room block:`
    )
  ...
  return (
    <div className="booking-calendar-page">
      {notice && <div className={`calendar-toast ${notice.type}`}>{notice.message}</div>}
              ...
              rooms.map((room) => (
                <div className="calendar-room-row" key={room.id}>
                  ...
                  >
                      ...
                      return (
                        <button
                          ...
                          onDragOver={(event) => {
                            ...
                            if (
                              dragTarget?.roomId !== room.id ||
                              dragTarget?.arrivalDate !== day
                            ) {
                              setDragTarget({
                                roomId: room.id,
                                roomNumber: room.room_number,
                          ...
                          onDrop={(event) => handleDrop(event, room.id, day)}
                          onDoubleClick={() =>
                            setBlockModal({
                              block: null,
                              initial: createBlockForm({
                    ...
                    {(eventsByRoom.get(room.id) || []).map((calendarEvent) => {
      ...
      {selectedEvent && (
        <EventDetailsDrawer
          ...
          onOpenStay={(guestSessionId) =>
            navigateToSection('checkin', { guestSessionId })
          }
          onReassign={(event) =>
      ...
      <aside className="calendar-details-drawer" role="dialog" aria-modal="true">
        ...
        <div className="calendar-details-body">
          ...
          <section className="calendar-detail-card">
            ...
            {isBlock && event.release_reason && <Detail label="Release reason" value={event.release_reason} wide />}
