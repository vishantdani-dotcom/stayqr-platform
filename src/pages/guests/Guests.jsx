...
export default function Guests({
  ...
  async function openSettlementModal(session) {
    ...
    try {
      ...
      const { data: issuedInvoice, error: issuedInvoiceError } = await supabase
        .from("invoices")
        .select("id, hotel_id, guest_session_id, folio_id, invoice_number, finalized_at, invoice_origin, metadata, subtotal_amount, tax_percent, tax_amount, discount_type, discount_value, discount_amount, total_amount")
        .eq("hotel_id", session.hotel_id)
        .eq("guest_session_id", session.id)
  ...
  async function completeFinalSettlement() {
    ...
    if ((await getCurrentHotel())?.id !== session.hotel_id) {
      showNotice("error", "The active hotel changed. Reopen checkout.");
      return;
    }
        ...
        settlementData && (
          <div style={modalOverlay} className="guest-final-checkout-overlay">
            <div style={settlementModal} className="guest-final-checkout-modal">
              ...
              {!settlementData.issuedInvoice && settlementCalculation.amountToCollect >
                0 && (
