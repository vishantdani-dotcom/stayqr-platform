...
-- StayQR Simple Front Desk REV6
-- Multi-occupant hardening for couples/families sharing one room.
-- 1) guest documents may attach to a stay when the guest is either the primary guest
--    or a persisted companion of that guest_session.
-- 2) check_in_walk_in_guest returns deterministic companion client_id -> guest_id mappings.
-- No UIDAI/government verification claim is introduced.
