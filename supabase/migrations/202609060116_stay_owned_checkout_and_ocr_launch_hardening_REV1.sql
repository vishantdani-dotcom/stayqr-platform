-- StayQR REV10 launch hardening: stay-owned checkout + room-move financial continuity.
-- OCR parser/fallback code is deployed separately with id-document-ocr.
begin;
set local lock_timeout = '10s';
set local statement_timeout = '180s';
select pg_advisory_xact_lock(hashtext('stayqr:202609060116:stay-owned-checkout'));

do $preflight$
begin
  if to_regprocedure('public.checkout_guest_session_day20_legacy(uuid,uuid,numeric,text,numeric,boolean,text,text,text,boolean)') is null
     or to_regprocedure('private.day20_checkout_existing_invoice(uuid,uuid,numeric,text,numeric,boolean,text,text,text,boolean)') is null then
    raise exception 'Required checkout functions are missing. No REV10 changes applied.';
  end if;
  if to_regclass('public.folios') is null or to_regclass('public.folio_items') is null
     or to_regclass('public.stay_room_history') is null then
    raise exception 'Authoritative stay billing tables are missing. No REV10 changes applied.';
  end if;
end;
$preflight$;
...
begin
  ...
  where fo.hotel_id = target_hotel_id
    ...
    and (
      fo.guest_session_id = target_guest_session_id
      or (
        fo.guest_session_id is null
        and fo.created_at >= session_row.checkin_time
        and exists (
          select 1 from public.stay_room_history srh
          where srh.hotel_id = target_hotel_id
            and srh.guest_session_id = target_guest_session_id
            and srh.room_id = fo.room_id
        )
      )
    )
  ...
  if invoice_row.metadata ->> 'day13_checkout_compatibility' = 'true' then
    ...
    raise exception 'Unknown issued invoice accounting format. Review Guest Bills before checkout.';
  end if;
  if folio_row.guest_session_id is distinct from target_guest_session_id
    ...
    raise exception 'Issued invoice and same-stay folio charges differ. Review Guest Bills before checkout.';
  end if;
  ...
  where fo.hotel_id = target_hotel_id
    ...
    and (
      ...
      or exists (
        ...
        where fi.hotel_id = target_hotel_id
          and fi.folio_id = invoice_row.folio_id
  ...
  update public.manual_charges mc
  set payment_status = 'paid'
  where mc.hotel_id = target_hotel_id
    ...
    and exists (
      ...
      where fi.hotel_id = target_hotel_id
        and fi.folio_id = invoice_row.folio_id
        and fi.source_table = 'manual_charges'
        and fi.source_id = mc.id
        and fi.posting_status = 'posted'
    );
...
begin
  ...
  if checkout_folio.id is null then
    raise exception 'Guest bill is not ready for this stay.';
  end if;

  -- REV10: the stay owns the bill. Room moves never detach financial charges.
  ...
  -- REV10 authoritative charge summary: posted folio lines belong to the stay,
  -- regardless of which room was active when the charge was created.
  select
    coalesce(sum(fi.amount) filter (where fi.charge_category = 'room'), 0),
    coalesce(sum(fi.amount) filter (where fi.charge_category = 'food'), 0),
    coalesce(sum(fi.amount) filter (where fi.charge_category in ('manual', 'other')), 0),
    coalesce(sum(fi.amount) filter (where fi.charge_category = 'service'), 0),
    count(distinct fi.source_id) filter (where fi.source_table = 'food_orders' and fi.source_id is not null)
  into room_amount, food_amount, manual_amount, service_amount, food_order_count
  from public.folio_items fi
  where fi.hotel_id = target_hotel_id
    and fi.folio_id = checkout_folio.id
    and fi.item_kind = 'charge'
    and fi.posting_status = 'posted';
  ...
  where exists (
    ...
    where fi.hotel_id = target_hotel_id
      ...
      and fi.posting_status = 'posted'
      and fi.source_table = 'food_orders'
      and fi.source_id = foi.order_id
  );

  subtotal := round(coalesce(checkout_folio.charges_amount, 0), 2);
  ...
  -- REV10 authoritative coverage comes from the stay folio. The pilot checkout
  -- contract currently requires refunds/credits to be resolved before checkout.
  previously_paid := round(coalesce(checkout_folio.collection_amount, 0), 2);
  folio_paid_amount := previously_paid;
  ...
  -- REV10 invoice lines are snapshotted from the authoritative folio so room
  -- moves preserve every room/food/service/manual charge on the final bill.
  insert into public.invoice_items (
    invoice_id, hotel_id, guest_id, room_id,
    item_type, description, quantity, unit_price, amount, source_id,
    folio_item_id, snapshot_json, metadata
  )
  select
    created_invoice_id,
    target_hotel_id,
    session_row.guest_id,
    session_row.room_id,
    case fi.charge_category
      when 'room' then 'room'
      when 'food' then 'food'
      when 'service' then 'service'
      when 'manual' then 'manual_charge'
      else 'other'
    end,
    fi.description,
    fi.quantity,
    ...
    jsonb_build_object(
      'source_table', fi.source_table,
      'service_at', fi.service_at,
      'charge_category', fi.charge_category
    ),
    jsonb_build_object('source', 'authoritative_folio', 'migration', '116')
  from public.folio_items fi
  where fi.hotel_id = target_hotel_id
    and fi.folio_id = checkout_folio.id
    and fi.item_kind = 'charge'
    and fi.posting_status = 'posted'
  order by fi.service_at, fi.created_at, fi.id;
...
comment on function public.checkout_guest_session_day20_legacy(uuid,uuid,numeric,text,numeric,boolean,text,text,text,boolean)
is 'REV10: final checkout is stay/folio-owned so charges survive room moves.';
comment on function private.day20_checkout_existing_invoice(uuid,uuid,numeric,text,numeric,boolean,text,text,text,boolean)
is 'REV10: existing invoice checkout blocks/settles same-stay orders across room moves.';
