...
-- Consolidated Commercial-Ready staging acceptance. Final result must be 52/52 TRUE.
with checks(test_name,passed,details) as (
  values
  ('01_provider_readiness_table',to_regclass('public.platform_provider_readiness') is not null,'Provider truth source exists.'),
  ('02_support_profile_table',to_regclass('public.stayqr_support_profile') is not null,'24x7 support profile exists.'),
  ('03_owner_requests_table',to_regclass('public.owner_subscription_requests') is not null,'Owner billing action ledger exists.'),
  ('04_recurring_attempts_table',to_regclass('public.subscription_recurring_attempts') is not null,'Recurring attempt evidence exists.'),
  ('05_uidai_requests_table',to_regclass('public.uidai_online_auth_requests') is not null,'UIDAI online request evidence exists.'),
  ('06_autopay_status_column',exists(select 1 from information_schema.columns where table_schema='public' and table_name='hotel_subscriptions' and column_name='autopay_status'),'Subscription AutoPay status exists.'),
  ('07_mandate_id_column',exists(select 1 from information_schema.columns where table_schema='public' and table_name='hotel_subscriptions' and column_name='mandate_id'),'Provider mandate ID exists.'),
  ('08_mandate_status_column',exists(select 1 from information_schema.columns where table_schema='public' and table_name='hotel_subscriptions' and column_name='mandate_status'),'Mandate status exists.'),
  ('09_next_charge_column',exists(select 1 from information_schema.columns where table_schema='public' and table_name='hotel_subscriptions' and column_name='next_charge_at'),'Next charge state exists.'),
  ('10_retry_count_column',exists(select 1 from information_schema.columns where table_schema='public' and table_name='hotel_subscriptions' and column_name='recurring_retry_count'),'Recurring retry counter exists.'),
  ('11_workspace_rpc',to_regprocedure('public.get_commercial_ready_workspace(uuid)') is not null,'Owner workspace RPC exists.'),
  ('12_owner_action_rpc',to_regprocedure('public.request_owner_subscription_action(uuid,text,uuid,text,text,text)') is not null,'Owner billing action RPC exists.'),
  ('13_provider_admin_rpc',to_regprocedure('public.set_commercial_provider_readiness(text,text,text,text,jsonb,jsonb)') is not null,'Provider readiness admin RPC exists.'),
  ('14_support_admin_rpc',to_regprocedure('public.configure_stayqr_support_profile(text,integer,integer)') is not null,'Support profile admin RPC exists.'),
  ('15_meta_admin_rpc',to_regprocedure('public.configure_meta_whatsapp_provider(uuid,text,text,text,text,jsonb)') is not null,'Meta sender configuration RPC exists.'),
  ('16_workspace_authenticated_execute',has_function_privilege('authenticated','public.get_commercial_ready_workspace(uuid)','execute'),'Hotel roles can load the protected workspace.'),
  ('17_owner_action_authenticated_execute',has_function_privilege('authenticated','public.request_owner_subscription_action(uuid,text,uuid,text,text,text)','execute'),'Authorized owners can submit billing actions.'),
  ('18_provider_admin_not_anon',not has_function_privilege('anon','public.set_commercial_provider_readiness(text,text,text,text,jsonb,jsonb)','execute'),'Anonymous users cannot activate providers.'),
  ('19_support_admin_not_anon',not has_function_privilege('anon','public.configure_stayqr_support_profile(text,integer,integer)','execute'),'Anonymous users cannot change support settings.'),
  ('20_owner_requests_rls',(select relrowsecurity from pg_class where oid='public.owner_subscription_requests'::regclass),'Owner requests use RLS.'),
  ('21_recurring_attempts_rls',(select relrowsecurity from pg_class where oid='public.subscription_recurring_attempts'::regclass),'Recurring attempts use RLS.'),
  ('22_uidai_requests_rls',(select relrowsecurity from pg_class where oid='public.uidai_online_auth_requests'::regclass),'UIDAI requests use RLS.'),
  ('23_owner_requests_no_authenticated_write',not has_table_privilege('authenticated','public.owner_subscription_requests','insert') and not has_table_privilege('authenticated','public.owner_subscription_requests','update') and not has_table_privilege('authenticated','public.owner_subscription_requests','delete'),'Browser cannot forge billing provider evidence.'),
  ('24_attempts_no_authenticated_write',not has_table_privilege('authenticated','public.subscription_recurring_attempts','insert') and not has_table_privilege('authenticated','public.subscription_recurring_attempts','update') and not has_table_privilege('authenticated','public.subscription_recurring_attempts','delete'),'Browser cannot forge recurring evidence.'),
  ('25_uidai_no_authenticated_write',not has_table_privilege('authenticated','public.uidai_online_auth_requests','insert') and not has_table_privilege('authenticated','public.uidai_online_auth_requests','update') and not has_table_privilege('authenticated','public.uidai_online_auth_requests','delete'),'Browser cannot forge UIDAI evidence.'),
  ('26_provider_settings_private',not has_table_privilege('authenticated','public.platform_provider_readiness','select') and not has_table_privilege('authenticated','public.platform_provider_readiness','insert') and not has_table_privilege('authenticated','public.platform_provider_readiness','update') and not has_table_privilege('authenticated','public.platform_provider_readiness','delete'),'Provider settings are RPC-only.'),
  ('27_support_settings_private',not has_table_privilege('authenticated','public.stayqr_support_profile','select') and not has_table_privilege('authenticated','public.stayqr_support_profile','insert') and not has_table_privilege('authenticated','public.stayqr_support_profile','update') and not has_table_privilege('authenticated','public.stayqr_support_profile','delete'),'Support contact is RPC-only.'),
  ('28_all_provider_rows',not exists(select 1 from (values('cashfree_recurring'),('meta_whatsapp'),('uidai_online')) v(k) where not exists(select 1 from public.platform_provider_readiness p where p.provider_key=v.k)),'All three provider readiness rows exist.'),
  ('29_no_fake_active_provider',not exists(select 1 from public.platform_provider_readiness where status='active' and (external_reference is null or metadata='{}'::jsonb)),'No provider is active without evidence.'),
  ('30_cashfree_truthful',exists(select 1 from public.platform_provider_readiness where provider_key='cashfree_recurring' and status in ('pending','in_review','sandbox_ready','active','suspended','failed')),'Cashfree readiness is explicit.'),
  ('31_meta_truthful',exists(select 1 from public.platform_provider_readiness where provider_key='meta_whatsapp' and status in ('pending','in_review','sandbox_ready','active','suspended','failed')),'Meta readiness is explicit.'),
  ('32_uidai_truthful',exists(select 1 from public.platform_provider_readiness where provider_key='uidai_online' and status in ('pending','in_review','sandbox_ready','active','suspended','failed')),'UIDAI readiness is explicit.'),
  ('33_support_24x7',exists(select 1 from public.stayqr_support_profile where singleton and coverage='24x7' and primary_channel='whatsapp'),'24x7 WhatsApp support policy is locked.'),
  ('34_after_hours_owner',exists(select 1 from public.stayqr_support_profile where singleton and after_hours_owner='Founder'),'Founder after-hours ownership is locked.'),
  ('35_support_number_not_faked',not exists(select 1 from public.stayqr_support_profile where support_whatsapp_e164 is not null and support_whatsapp_e164 !~ '^\+[1-9][0-9]{7,14}$'),'Configured support number is valid E.164 or pending.'),
  ('36_online_consent_allowed',exists(select 1 from pg_constraint where conname='guest_consents_purpose_check' and pg_get_constraintdef(oid) like '%aadhaar_online_authentication%'),'Online Aadhaar consent is explicit.'),
  ('37_online_verification_method',exists(select 1 from pg_constraint where conname='guest_identity_verifications_method_check' and pg_get_constraintdef(oid) like '%aadhaar_online_auth%'),'Online verification evidence method is allowed.'),
  ('38_uidai_no_aadhaar_column',not exists(select 1 from information_schema.columns where table_schema='public' and table_name='uidai_online_auth_requests' and column_name in ('aadhaar_number','aadhaar','otp','pid','biometric')),'Aadhaar/OTP/PID are not persistence columns.'),
  ('39_uidai_hash_required',exists(select 1 from information_schema.columns where table_schema='public' and table_name='uidai_online_auth_requests' and column_name='aadhaar_sha256' and is_nullable='NO'),'UIDAI evidence requires a non-reversible hash.'),
  ('40_uidai_last4_only',exists(select 1 from information_schema.columns where table_schema='public' and table_name='uidai_online_auth_requests' and column_name='aadhaar_last4'),'Only last four may be displayed.'),
  ('41_owner_idempotency_unique',exists(select 1 from pg_indexes where schemaname='public' and tablename='owner_subscription_requests' and indexdef ilike '%idempotency_key%' and indexdef ilike '%unique%'),'Owner billing actions are idempotent.'),
  ('42_provider_event_unique',exists(select 1 from pg_indexes where schemaname='public' and tablename='subscription_recurring_attempts' and indexname='uq_recurring_attempt_provider_event'),'Recurring webhook evidence is exact-once.'),
  ('43_owner_recent_index',exists(select 1 from pg_indexes where schemaname='public' and indexname='idx_owner_subscription_requests_hotel_recent'),'Owner history is indexed.'),
  ('44_attempt_recent_index',exists(select 1 from pg_indexes where schemaname='public' and indexname='idx_recurring_attempt_hotel_recent'),'Recurring history is indexed.'),
  ('45_uidai_recent_index',exists(select 1 from pg_indexes where schemaname='public' and indexname='idx_uidai_online_requests_hotel_guest_recent'),'UIDAI evidence is indexed.'),
  ('46_autopay_constraint',exists(select 1 from pg_constraint where conname='hotel_subscriptions_autopay_status_cr_check'),'AutoPay states are constrained.'),
  ('47_retry_constraint',exists(select 1 from pg_constraint where conname='hotel_subscriptions_recurring_retry_cr_check'),'Retry count is bounded.'),
  ('48_owner_action_constraint',exists(select 1 from pg_constraint where conname='owner_subscription_action_cr_check'),'Owner actions are constrained.'),
  ('49_provider_status_constraint',exists(select 1 from pg_constraint where conname='platform_provider_status_cr_check'),'Provider states are constrained.'),
  ('50_uidai_status_constraint',exists(select 1 from pg_constraint where conname='uidai_online_status_cr_check'),'UIDAI states are constrained.'),
  ('51_service_role_provider_write',has_table_privilege('service_role','public.platform_provider_readiness','select') and has_table_privilege('service_role','public.platform_provider_readiness','insert') and has_table_privilege('service_role','public.platform_provider_readiness','update') and has_table_privilege('service_role','public.platform_provider_readiness','delete'),'Trusted provider reconciliation can write.'),
  ('52_service_role_evidence_write',has_table_privilege('service_role','public.subscription_recurring_attempts','select') and has_table_privilege('service_role','public.subscription_recurring_attempts','insert') and has_table_privilege('service_role','public.subscription_recurring_attempts','update') and has_table_privilege('service_role','public.uidai_online_auth_requests','select') and has_table_privilege('service_role','public.uidai_online_auth_requests','insert') and has_table_privilege('service_role','public.uidai_online_auth_requests','update'),'Trusted Edge Functions can write evidence.')
)
